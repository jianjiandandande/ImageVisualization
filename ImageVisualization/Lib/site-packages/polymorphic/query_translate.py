...
def translate_polymorphic_field_path(queryset_model, field_path):
    ...
    if '__' in classname:
        # the user has apps label prepended to class name via __ => use Django's get_model function
        appname, sep, classname = classname.partition('__')
        model = apps.get_model(appname, classname)
        assert model, 'PolymorphicModel: model %s (in apps %s) not found!' % (model.__name__, appname)
